  ...
  async saveUserProfile(profile: any, userId: string, isOnline: boolean) {
    ...
    const cloudPayload = {
      ...
      persona_data: {
        ...
        mascotXp: sanitized.mascotXp ?? sanitized.mascot_xp ?? 0,
        mascotLevel: sanitized.mascotLevel ?? sanitized.mascot_level ?? 1,
    ...
    if (isOnline) {
      ...
      if (!error && data) {
        const profile = sanitizeProfile({
          ...
          mascotXp: data.persona_data?.mascotXp ?? data.persona_data?.mascot_xp ?? 0,
          mascotLevel: data.persona_data?.mascotLevel ?? data.persona_data?.mascot_level ?? 1,
          mascot_xp: data.persona_data?.mascotXp ?? data.persona_data?.mascot_xp ?? 0,
          mascot_level: data.persona_data?.mascotLevel ?? data.persona_data?.mascot_level ?? 1,
