...
import { LayoutDashboard, Timer, BookOpen, CheckSquare, Menu, X, BrainCircuit, Moon, Sun, LogOut } from 'lucide-react';
...
const App: React.FC = () => {
  ...
  const renderContent = () => {
    ...
    switch (currentView) {
      ...
      case View.Timer: return <Pomodoro subjects={subjects} />;
    ...
    <div className={`flex h-screen overflow-hidden transition-colors duration-300 ${isDarkMode ? 'dark bg-sanfran-rubiBlack' : 'bg-[#fcfcfc]'}`}>
      {!isSidebarOpen && (
        <button 
          ...
          className="fixed top-4 left-4 z-50 p-3 bg-white dark:bg-sanfran-rubiDark rounded-xl shadow-xl lg:hidden border border-slate-200 dark:border-sanfran-rubi/20"
        >
          <Menu className="w-6 h-6 text-sanfran-rubi" />
        </button>
      )}

      <aside className={`
        ${isSidebarOpen ? 'translate-x-0' : '-translate-x-full'}
        fixed inset-y-0 left-0 z-40 w-64 bg-white dark:bg-[#0d0303] border-r border-slate-200 dark:border-sanfran-rubi/30 transition-all duration-300 ease-in-out lg:relative lg:translate-x-0 shadow-2xl lg:shadow-none flex flex-col
      `}>
        <div className="flex items-center justify-between p-6 border-b border-slate-100 dark:border-sanfran-rubi/20">
          <div className="flex items-center gap-3">
            <div className="bg-sanfran-rubi p-2 rounded-xl text-white shadow-lg">
          ...
          <button onClick={() => setIsSidebarOpen(false)} className="lg:hidden text-slate-400 dark:text-sanfran-rubi">
            <X className="w-6 h-6" />
          </button>
        </div>
          ...
          {navItems.map((item) => (
            <button
              ...
              className={`
                ...
                ${currentView === item.id 
                  ? 'bg-sanfran-rubi text-white font-black shadow-md shadow-red-900/20' 
                  : 'text-slate-700 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-sanfran-rubi/10 hover:text-sanfran-rubi dark:hover:text-white'}
              `}
            >
        ...
        <div className="p-4 space-y-3">
          <button 
            ...
            className="w-full flex items-center justify-between px-5 py-4 rounded-2xl bg-slate-100 dark:bg-sanfran-rubiDark border border-slate-200 dark:border-sanfran-rubi/30 text-slate-950 dark:text-white hover:scale-[1.02] transition-all shadow-sm"
          >
          ...
          <button 
            ...
            className="w-full flex items-center gap-3 px-5 py-3 rounded-2xl text-slate-500 hover:text-red-600 dark:text-slate-500 dark:hover:text-red-400 transition-all font-black uppercase text-[10px] tracking-widest"
          >
