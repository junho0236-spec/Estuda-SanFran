...
const Dashboard: React.FC<DashboardProps> = ({ subjects, flashcards, tasks, studySessions, readings, onNavigate }) => {
  ...
  return (
    <div className="space-y-8 md:space-y-12 animate-in fade-in duration-500 pb-20">
      ...
      {/* --- NOVO: Escritório Virtual --- */}
      <VirtualOffice totalHours={totalHours} />

      {/* --- Seção de Status (Antiga Seção de Carreira compactada) --- */}
      <div className="bg-white dark:bg-sanfran-rubiDark/30 rounded-[2rem] p-6 border border-slate-200 dark:border-sanfran-rubi/30 shadow-lg flex items-center justify-between gap-4">
