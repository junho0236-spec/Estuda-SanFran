import Layout from "@forja/components/Layout";
    ...
    <Layout>
      <div className="px-4 py-4 max-w-2xl mx-auto space-y-4">
        ...
        <div className="flex items-center gap-3">
          <div className="w-10 h-10 bg-sanfran-rubi rounded-xl flex items-center justify-center">
            ...
            {subTabs.map(tab => {
              ...
              return (
                <button key={tab.id} onClick={() => setActiveSubTab(tab.id)} title={tab.label}
                  className={`w-10 h-10 rounded-xl flex items-center justify-center transition ${activeSubTab === tab.id ? 'bg-sanfran-rubi text-white' : 'bg-sanfran-paper dark:bg-slate-800 text-slate-500 dark:text-slate-400 hover:bg-slate-200 dark:hover:bg-slate-700'}`}>
          ...
          <>
            <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-2xl p-5">
              <div className="flex items-center justify-between mb-4">
                <button onClick={() => setCalYear(y => y - 1)} className="p-2 hover:bg-slate-100 dark:hover:bg-slate-800 rounded-lg transition"><ChevronLeft className="w-4 h-4" /></button>
                <h3 className="font-bold">Saldo por Mês — {calYear}</h3>
                <button onClick={() => setCalYear(y => y + 1)} className="p-2 hover:bg-slate-100 dark:hover:bg-slate-800 rounded-lg transition"><ChevronRight className="w-4 h-4" /></button>
              </div>
            ...
            <div className="grid grid-cols-2 gap-3">
              {monthNames.map((month, idx) => {
                ...
                return (
                  <div key={month} className={`bg-white dark:bg-slate-900 border rounded-2xl p-4 ${isCurrentMonth ? 'border-sanfran-rubi/40' : 'border-slate-200 dark:border-slate-700'}`}>
                    <div className="flex items-center justify-between mb-3">
                      <h4 className="font-bold text-sm">{month}</h4>
                      {isCurrentMonth && <span className="text-xs text-sanfran-rubi font-medium">Atual</span>}
                    </div>
                    <div className="space-y-1.5">
                      ...
                      <div className="border-t border-slate-200 dark:border-slate-700 pt-1.5 flex justify-between"><span className="text-xs text-gray-400">Saldo</span><span className={`text-xs font-bold ${(mIncome - mExpense) >= 0 ? 'text-green-400' : 'text-red-500'}`}>R$ {(mIncome - mExpense).toFixed(2)}</span></div>
          ...
          <div className="space-y-4">
            {cards.length === 0 ? (
              <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-2xl p-8 text-center">
                ...
                <button onClick={() => openModal("card")} className="text-sanfran-rubi text-sm font-medium">+ Adicionar cartão</button>
              </div>
            ) : (
              cards.map((card: any) => (
            ...
            <button onClick={() => openModal("card")} className="w-full bg-white dark:bg-slate-900 border border-dashed border-slate-300 dark:border-slate-600 rounded-2xl p-4 text-gray-400 hover:border-sanfran-rubi hover:text-sanfran-rubi transition flex items-center justify-center gap-2">
          ...
          <div className="space-y-4">
            ...
            {accounts.length === 0 ? (
              <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-2xl p-8 text-center">
                ...
                <button onClick={() => openModal("account")} className="text-sanfran-rubi text-sm font-medium">+ Adicionar conta</button>
              </div>
            ) : (
              accounts.map((acc: any) => (
                <div key={acc.id} className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-2xl p-4 flex items-center justify-between">
                  <div className="flex items-center gap-3">
            ...
            <button onClick={() => openModal("account")} className="w-full bg-white dark:bg-slate-900 border border-dashed border-slate-300 dark:border-slate-600 rounded-2xl p-4 text-gray-400 hover:border-sanfran-rubi hover:text-sanfran-rubi transition flex items-center justify-center gap-2">
          ...
          <div className="space-y-4">
            {shopping.length === 0 ? (
              <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-2xl p-8 text-center">
                ...
                <button onClick={() => openModal("shopping")} className="text-sanfran-rubi text-sm font-medium">+ Adicionar item</button>
              </div>
            ) : (
              <>
                {shopping.filter((s: any) => s.status === "pending").length > 0 && (
                  <div>
                    ...
                    {shopping.filter((s: any) => s.status === "pending").map((item: any) => (
                      <div key={item.id} className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-xl p-4 mb-2 flex items-center justify-between">
                        <div className="flex items-center gap-3">
                ...
                {shopping.filter((s: any) => s.status === "bought").length > 0 && (
                  <div>
                    ...
                    {shopping.filter((s: any) => s.status === "bought").map((item: any) => (
                      <div key={item.id} className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-xl p-4 mb-2 flex items-center justify-between opacity-60">
                        <div className="flex items-center gap-3">
            ...
            <button onClick={() => openModal("shopping")} className="w-full bg-white dark:bg-slate-900 border border-dashed border-slate-300 dark:border-slate-600 rounded-2xl p-4 text-gray-400 hover:border-sanfran-rubi hover:text-sanfran-rubi transition flex items-center justify-center gap-2">
                  ...
                  {categoryData.map((cat, i) => {
                    ...
                    return (
                      <div key={cat.name}>
                        ...
                        <div className="w-full bg-sanfran-paper dark:bg-slate-800 rounded-full h-2">
          ...
          <div className="space-y-4">
            {rules.length === 0 ? (
              <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-2xl p-8 text-center">
                ...
                <button onClick={() => openModal("rule")} className="text-sanfran-rubi text-sm font-medium">+ Criar regra</button>
              </div>
            ) : (
              rules.map((rule: any) => {
                ...
                return (
                  <div key={rule.id} className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-2xl p-4">
                    ...
                    <div className="w-full bg-sanfran-paper dark:bg-slate-800 rounded-full h-3 mb-2">
            ...
            <button onClick={() => openModal("rule")} className="w-full bg-white dark:bg-slate-900 border border-dashed border-slate-300 dark:border-slate-600 rounded-2xl p-4 text-gray-400 hover:border-sanfran-rubi hover:text-sanfran-rubi transition flex items-center justify-center gap-2">
        ...
        {activeSubTab === "monitor" && (
          <div className="space-y-4">
            ...
            <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-2xl p-5">
              ...
              {rules.length === 0 ? (
                <p className="text-gray-400 text-sm">Nenhuma regra configurada. <button type="button" onClick={() => setActiveSubTab("rules")} className="text-sanfran-rubi font-medium">Criar regra</button></p>
              ) : (
            ...
            <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-2xl p-5">
              ...
              {cards.length === 0 ? <p className="text-gray-400 text-sm">Nenhum cartão cadastrado. <button onClick={() => setActiveSubTab("cards")} className="text-red-400">Adicionar</button></p> : (
                <div className="space-y-2">
                  {cards.map((card: any) => (
                    <div key={card.id} className="flex items-center justify-between p-3 bg-sanfran-paper dark:bg-slate-800 rounded-xl">
          ...
          <div className="space-y-4">
            {notes.length === 0 ? (
              <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-2xl p-8 text-center">
                ...
                <button onClick={() => openModal("note")} className="text-sanfran-rubi text-sm font-medium">+ Criar nota</button>
              </div>
            ) : (
              <div className="grid grid-cols-2 gap-3">
                {notes.map((note: any) => (
            ...
            <button onClick={() => openModal("note")} className="w-full bg-white dark:bg-slate-900 border border-dashed border-slate-300 dark:border-slate-600 rounded-2xl p-4 text-gray-400 hover:border-sanfran-rubi hover:text-sanfran-rubi transition flex items-center justify-center gap-2">
          ...
          <div className="space-y-4">
            {accounts.length < 2 ? (
              <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-2xl p-8 text-center">
                ...
                <button onClick={() => setActiveSubTab("bank")} className="text-sanfran-rubi text-sm font-medium">Ir para Contas</button>
              </div>
            ) : (
                  ...
                  <div className="space-y-2">
                    {accounts.map((acc: any) => (
                      <button key={acc.id} onClick={() => { setTransferFrom(acc.id); if (transferTo === acc.id) setTransferTo(null); }}
                        className={`w-full p-3 rounded-xl flex items-center justify-between transition ${transferFrom === acc.id ? 'bg-red-500/20 border border-red-500' : 'bg-sanfran-paper dark:bg-slate-800 border border-transparent hover:border-slate-300 dark:border-slate-600'}`}>
                  ...
                  <div className="space-y-2">
                    {accounts.filter((a: any) => a.id !== transferFrom).map((acc: any) => (
                      <button key={acc.id} onClick={() => setTransferTo(acc.id)}
                        className={`w-full p-3 rounded-xl flex items-center justify-between transition ${transferTo === acc.id ? 'bg-green-500/20 border border-green-500' : 'bg-sanfran-paper dark:bg-slate-800 border border-transparent hover:border-slate-300 dark:border-slate-600'}`}>
                        <span>{acc.icon} {acc.name}</span>
              ...
              <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-2xl p-5">
                ...
                <div className="space-y-2">
                  {allTransactions.filter((t: any) => t.category === "Transferência").slice(0, 10).map((tx: any) => (
                    <div key={tx.id} className="flex items-center justify-between p-3 bg-sanfran-paper dark:bg-slate-800 rounded-xl">
                      <div className="flex items-center gap-2">
          ...
          <div className="space-y-3">
            {allTransactions.filter((t: any) => t.category !== "Transferência").length > 0 ? (
              allTransactions.filter((t: any) => t.category !== "Transferência").map((tx: any) => (
                <div key={tx.id} className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-xl p-4 flex items-center justify-between">
                  <div className="flex items-center gap-3">
            ...
            <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-2xl p-5">
              ...
              {rules.length === 0 ? (
                <div className="text-center py-4">
                  ...
                  <button onClick={() => setActiveSubTab("rules")} className="text-sanfran-rubi text-sm font-medium">Ir para Regras</button>
                </div>
              ) : (
                <>
                  <div className="mb-4">
                    ...
                    <div className="w-full bg-sanfran-paper dark:bg-slate-800 rounded-full h-4 flex overflow-hidden">
                  ...
                  <div className="space-y-2">
                    {rules.map((rule: any, i: number) => {
                      ...
                      return (
                        <div key={rule.id} className="flex items-center justify-between p-3 bg-sanfran-paper dark:bg-slate-800 rounded-xl">
                          <div className="flex items-center gap-2">
        ...
        <div className="fixed inset-0 z-50 bg-black/70 flex items-end justify-center" onClick={closeModal}>
          <div className="bg-white dark:bg-slate-900 w-full max-w-lg rounded-t-3xl p-6 max-h-[90vh] overflow-y-auto" onClick={e => e.stopPropagation()}>
            <div className="flex items-center justify-between mb-6">
              ...
              <button onClick={closeModal} className="p-2 hover:bg-slate-100 dark:hover:bg-slate-800 rounded-lg"><X className="w-5 h-5" /></button>
            </div>
              ...
              <div className="space-y-5">
                <div>
                  ...
                  <div className="flex gap-2">
                    <button onClick={() => setNewType("expense")} className={`flex-1 py-2.5 rounded-xl text-sm font-medium transition ${newType === "expense" ? 'bg-red-500 text-white' : 'bg-sanfran-paper dark:bg-slate-800 text-gray-400 hover:bg-[#333]'}`}>Despesa</button>
                    <button onClick={() => setNewType("income")} className={`flex-1 py-2.5 rounded-xl text-sm font-medium transition ${newType === "income" ? 'bg-green-500 text-white' : 'bg-sanfran-paper dark:bg-slate-800 text-gray-400 hover:bg-[#333]'}`}>Receita</button>
                  </div>
                </div>
                <div>
                ...
                <div>
                  ...
                  <div className="flex gap-2 flex-wrap">
                    {expenseCategories.map(cat => (
                      <button key={cat} onClick={() => setNewCategory(cat)} className={`px-3 py-2 rounded-xl text-sm transition ${newCategory === cat ? 'bg-red-500 text-white' : 'bg-sanfran-paper dark:bg-slate-800 text-gray-400 hover:bg-[#333]'}`}>{cat}</button>
                    ))}
                  </div>
                </div>
                <div>
                ...
                <div className="flex gap-3 pt-2">
                  <button onClick={closeModal} className="flex-1 bg-sanfran-paper dark:bg-slate-800 text-gray-300 py-3 rounded-xl font-medium hover:bg-[#333] transition">Cancelar</button>
            ...
            {modalType === "card" && (
              <div className="space-y-4">
                ...
                <div className="flex gap-2">
                  <button onClick={() => setCardType("credit")} className={`flex-1 py-2.5 rounded-xl text-sm font-medium transition ${cardType === "credit" ? 'bg-red-500 text-white' : 'bg-sanfran-paper dark:bg-slate-800 text-gray-400'}`}>Crédito</button>
                  <button onClick={() => setCardType("debit")} className={`flex-1 py-2.5 rounded-xl text-sm font-medium transition ${cardType === "debit" ? 'bg-blue-500 text-white' : 'bg-sanfran-paper dark:bg-slate-800 text-gray-400'}`}>Débito</button>
                </div>
                <div>
                  <label className="text-sm text-gray-400 mb-2 block">Bandeira</label>
                  <div className="flex gap-2 flex-wrap">
                    {["Visa", "Master", "Elo", "Amex", "Hipercard"].map(b => (
                      <button key={b} onClick={() => setCardBrand(b)} className={`px-3 py-1.5 rounded-lg text-xs transition ${cardBrand === b ? 'bg-red-500 text-white' : 'bg-sanfran-paper dark:bg-slate-800 text-gray-400'}`}>{b}</button>
                    ))}
                  </div>
                </div>
                <div className="grid grid-cols-2 gap-3">
                ...
                <div>
                  ...
                  <div className="flex gap-2">
                    {["#EF4444", "#8B5CF6", "#3B82F6", "#10B981", "#F59E0B", "#EC4899"].map(c => (
                      <button key={c} onClick={() => setCardColor(c)} className={`w-8 h-8 rounded-full transition ${cardColor === c ? 'ring-2 ring-white ring-offset-2 ring-offset-white dark:ring-offset-slate-900' : ''}`} style={{ backgroundColor: c }} />
                    ))}
                  </div>
                </div>
                <div className="flex gap-3 pt-2">
                ...
                <div>
                  ...
                  <div className="flex gap-2 flex-wrap">
                    {(["checking", "savings", "investment", "wallet"] as const).map(t => (
                      <button key={t} onClick={() => setAccType(t)} className={`px-3 py-2 rounded-xl text-sm transition ${accType === t ? 'bg-red-500 text-white' : 'bg-sanfran-paper dark:bg-slate-800 text-gray-400'}`}>{accountTypeLabels[t]}</button>
                    ))}
                  </div>
                </div>
                <div>
                  ...
                  <div className="flex gap-2 flex-wrap">
                    {["🏦", "💳", "💰", "🏧", "📱", "💵", "🪙", "📊"].map(ic => (
                      <button key={ic} onClick={() => setAccIcon(ic)} className={`w-10 h-10 rounded-xl text-xl flex items-center justify-center transition ${accIcon === ic ? 'bg-red-500/30 ring-2 ring-red-500' : 'bg-sanfran-paper dark:bg-slate-800 hover:bg-[#333]'}`}>{ic}</button>
                    ))}
                  </div>
                </div>
                <div>
                ...
                <div>
                  ...
                  <div className="flex gap-2">
                    {["#3B82F6", "#EF4444", "#10B981", "#8B5CF6", "#F59E0B", "#EC4899"].map(c => (
                      <button key={c} onClick={() => setAccColor(c)} className={`w-8 h-8 rounded-full transition ${accColor === c ? 'ring-2 ring-white ring-offset-2 ring-offset-white dark:ring-offset-slate-900' : ''}`} style={{ backgroundColor: c }} />
                    ))}
                  </div>
                </div>
                <div className="flex gap-3 pt-2">
                ...
                <div>
                  ...
                  <div className="flex gap-2">
                    {([["low", "Baixa"], ["medium", "Média"], ["high", "Alta"]] as const).map(([val, label]) => (
                      <button key={val} onClick={() => setShopPriority(val)} className={`flex-1 py-2 rounded-xl text-sm transition ${shopPriority === val ? (val === 'high' ? 'bg-red-500 text-white' : val === 'medium' ? 'bg-yellow-500 text-white' : 'bg-gray-500 text-white') : 'bg-sanfran-paper dark:bg-slate-800 text-gray-400'}`}>{label}</button>
                    ))}
                  </div>
                </div>
                <div>
            ...
            {modalType === "rule" && (
              <div className="space-y-4">
                <div>
                  ...
                  <div className="flex gap-2 flex-wrap">
                    {expenseCategories.map(cat => (
                      <button key={cat} onClick={() => setRuleCategory(cat)} className={`px-3 py-2 rounded-xl text-sm transition ${ruleCategory === cat ? 'bg-red-500 text-white' : 'bg-sanfran-paper dark:bg-slate-800 text-gray-400'}`}>{cat}</button>
                    ))}
                  </div>
                </div>
                <div>
              ...
              <div className="space-y-4">
                ...
                <div>
                  ...
                  <textarea value={noteContent} onChange={e => setNoteContent(e.target.value)} placeholder="Escreva sua nota..."
                    className="w-full bg-sanfran-paper dark:bg-slate-800 border border-[#333] rounded-xl px-4 py-3 text-white placeholder-gray-500 focus:border-red-500 focus:outline-none transition h-24 resize-none" />
                </div>
                <div>
                  <label className="text-sm text-gray-400 mb-2 block">Cor</label>
                  <div className="flex gap-2">
                    {["#FBBF24", "#EF4444", "#10B981", "#3B82F6", "#8B5CF6", "#EC4899"].map(c => (
                      <button key={c} onClick={() => setNoteColor(c)} className={`w-8 h-8 rounded-full transition ${noteColor === c ? 'ring-2 ring-white ring-offset-2 ring-offset-white dark:ring-offset-slate-900' : ''}`} style={{ backgroundColor: c }} />
                    ))}
                  </div>
                </div>
                <div className="flex gap-3 pt-2">
