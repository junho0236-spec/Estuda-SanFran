  ...
  if (isFullscreen) {
    return (
      <div className="fixed inset-0 z-[100] bg-black flex flex-col items-center justify-center">
        <button onClick={toggleFullscreen} className="absolute top-6 right-6 p-2 bg-white dark:bg-slate-900 rounded-lg hover:bg-slate-100 dark:hover:bg-slate-800 transition">
          <Minus className="w-5 h-5" />
        </button>
        <p className="text-sm text-gray-400 mb-4">{isFocus ? 'FOCO' : 'PAUSA'}</p>
        <div className="relative w-64 h-64">
          <svg className="w-full h-full -rotate-90" viewBox="0 0 100 100">
            <circle cx="50" cy="50" r="45" fill="none" stroke="#94a3b8" strokeWidth="3" />
        ...
        <div className="flex gap-4 mt-8">
          <button onClick={resetTimer} className="w-14 h-14 bg-sanfran-paper dark:bg-slate-800 rounded-full flex items-center justify-center hover:bg-[#333] transition">
    ...
    <Layout>
      <div className="px-4 py-4 max-w-2xl mx-auto space-y-4">
        ...
        <div className="flex items-center justify-between">
          ...
          <div className="flex gap-2">
            <button onClick={toggleFullscreen} className="p-2 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-xl hover:bg-slate-100 dark:hover:bg-slate-800 transition">
              <Maximize className="w-5 h-5" />
            </button>
            <button onClick={() => setShowHistory(!showHistory)} className="p-2 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-xl hover:bg-slate-100 dark:hover:bg-slate-800 transition">
        ...
        <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-2xl p-6">
          <div className="flex justify-center gap-3 mb-6">
            <button onClick={() => { setIsFocus(true); if (!isRunning) setTimeLeft(focusTime * 60); }}
              className={`px-4 py-2 rounded-xl text-sm font-medium transition ${isFocus ? 'bg-red-500 text-white' : 'bg-sanfran-paper dark:bg-slate-800 text-gray-400'}`}>
              Foco
            </button>
            <button onClick={() => { setIsFocus(false); if (!isRunning) setTimeLeft(breakTime * 60); }}
              className={`px-4 py-2 rounded-xl text-sm font-medium transition ${!isFocus ? 'bg-green-500 text-white' : 'bg-sanfran-paper dark:bg-slate-800 text-gray-400'}`}>
              Pausa
            </button>
          </div>
          ...
          <div className="flex justify-center mb-6">
            <div className="relative w-48 h-48">
              <svg className="w-full h-full -rotate-90" viewBox="0 0 100 100">
                <circle cx="50" cy="50" r="45" fill="none" stroke="#94a3b8" strokeWidth="4" />
          ...
          <div className="flex justify-center gap-4 mb-4">
            <button onClick={resetTimer} className="w-12 h-12 bg-sanfran-paper dark:bg-slate-800 rounded-full flex items-center justify-center hover:bg-[#333] transition">
          ...
          <div className="flex justify-center gap-6 text-sm">
            <div className="flex items-center gap-2">
              ...
              <button onClick={() => { setFocusTime(Math.max(5, focusTime - 5)); if (isFocus && !isRunning) setTimeLeft(Math.max(5, focusTime - 5) * 60); }} className="w-7 h-7 bg-sanfran-paper dark:bg-slate-800 rounded flex items-center justify-center hover:bg-[#333]">-</button>
              <span className="font-bold w-8 text-center">{focusTime}</span>
              <button onClick={() => { setFocusTime(focusTime + 5); if (isFocus && !isRunning) setTimeLeft((focusTime + 5) * 60); }} className="w-7 h-7 bg-sanfran-paper dark:bg-slate-800 rounded flex items-center justify-center hover:bg-[#333]">+</button>
            </div>
            <div className="flex items-center gap-2">
              <span className="text-gray-400">Pausa:</span>
              <button onClick={() => { setBreakTime(Math.max(1, breakTime - 1)); if (!isFocus && !isRunning) setTimeLeft(Math.max(1, breakTime - 1) * 60); }} className="w-7 h-7 bg-sanfran-paper dark:bg-slate-800 rounded flex items-center justify-center hover:bg-[#333]">-</button>
              <span className="font-bold w-8 text-center">{breakTime}</span>
              <button onClick={() => { setBreakTime(breakTime + 1); if (!isFocus && !isRunning) setTimeLeft((breakTime + 1) * 60); }} className="w-7 h-7 bg-sanfran-paper dark:bg-slate-800 rounded flex items-center justify-center hover:bg-[#333]">+</button>
            </div>
          </div>
        </div>
        ...
        <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-2xl p-4">
          <div className="flex items-center justify-between">
            ...
            <button onClick={() => setShowProjectModal(true)} className="p-2 bg-sanfran-paper dark:bg-slate-800 rounded-lg hover:bg-[#333] transition">
          ...
          {showProjectSelector && (
            <div className="mt-3 space-y-2">
              <button onClick={() => { setSelectedProjectId(null); setShowProjectSelector(false); }}
                className={`w-full text-left px-3 py-2 rounded-lg text-sm transition ${!selectedProjectId ? 'bg-sanfran-paper dark:bg-slate-800 text-white' : 'text-gray-400 hover:bg-slate-100 dark:hover:bg-slate-800'}`}>
                Sem projeto
              </button>
              {projects.map((p: any) => (
                <div key={p.id} className="flex items-center justify-between">
                  <button onClick={() => { setSelectedProjectId(p.id); setShowProjectSelector(false); }}
                    className={`flex-1 text-left px-3 py-2 rounded-lg text-sm transition flex items-center gap-2 ${selectedProjectId === p.id ? 'bg-sanfran-paper dark:bg-slate-800 text-white' : 'text-gray-400 hover:bg-slate-100 dark:hover:bg-slate-800'}`}>
            ...
            {sessions.length > 0 ? (
              <div className="space-y-2 max-h-60 overflow-y-auto">
                {sessions.slice(0, 20).map((s: any) => (
                  <div key={s.id} className="flex items-center justify-between bg-sanfran-paper dark:bg-slate-800 rounded-lg px-3 py-2">
                    <div className="flex items-center gap-2">
      ...
      {showProjectModal && (
        <div className="fixed inset-0 z-50 bg-black/70 flex items-end justify-center" onClick={() => setShowProjectModal(false)}>
          <div className="bg-white dark:bg-slate-900 w-full max-w-lg rounded-t-3xl p-6" onClick={e => e.stopPropagation()}>
            <div className="flex items-center justify-between mb-6">
              <h2 className="text-xl font-bold">Novo Projeto</h2>
              <button onClick={() => setShowProjectModal(false)} className="p-2 hover:bg-slate-100 dark:hover:bg-slate-800 rounded-lg"><X className="w-5 h-5" /></button>
            </div>
                ...
                <div className="flex gap-2">
                  {projectColors.map(c => (
                    <button key={c} onClick={() => setNewProjectColor(c)}
                      className={`w-10 h-10 rounded-lg transition ${newProjectColor === c ? 'ring-2 ring-white ring-offset-2 ring-offset-white dark:ring-offset-slate-900' : ''}`}
                      style={{ backgroundColor: c }}></button>
                  ))}
                </div>
              </div>
              <div className="flex gap-3 pt-2">
                <button onClick={() => setShowProjectModal(false)} className="flex-1 bg-sanfran-paper dark:bg-slate-800 text-gray-300 py-3 rounded-xl font-medium hover:bg-[#333] transition">Cancelar</button>
                <button onClick={() => {
