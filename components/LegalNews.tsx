...
import { Search, MapPin, Scale, Navigation, AlertTriangle, Loader2, Bookmark, Sparkles } from 'lucide-react';
    ...
    <div className="max-w-4xl mx-auto space-y-8 animate-in fade-in duration-700 pb-20">
      <header className="flex flex-col md:flex-row md:items-end justify-between gap-6">
        ...
        <div className="flex bg-slate-100 dark:bg-white/5 p-1 rounded-2xl border border-slate-200 dark:border-white/10">
           ...
           <button onClick={() => { setActiveTab('courts'); setResults(null); }} className={`px-6 py-2 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all ${activeTab === 'courts' ? 'bg-emerald-500 text-white shadow-lg' : 'text-slate-400 hover:text-slate-600'}`}>Tribunais</button>
        </div>
      </header>
